...
\subsubsection{Transfert on the second and third images}
...
\begin{center}
 $ \displaystyle { \sum_{k = 1}^{3}} x_{p}^{k}(x_{p}^{'i} ( x_{p}^{''3}T_{k}^{3l} - x_{p}^{''l}T_{k}^{33}) + x_{p}^{'3}( x_{p}^{''l}T_{k}^{i3} - x_{p}^{''3}T_{k}^{il} ))$
\end{center}

Then we have for B and b :  
\begin{center}
$B_{i}^{il} = x_{p}^{k}(x_{p}^{''3}T_{k}^{3l} - x_{p}^{''l}T_{k}^{33})$
\end{center}
\begin{center}
$b^{il} = x_{p}^{k}(x_{p}^{''l}T_{k}^{i3} - x_{p}^{''3}T_{k}^{il})$
\end{center}
...
The method to get the x is the same than is the first transfert. 
